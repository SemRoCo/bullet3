...
bool PhysicsDirect::getSoftBodyConfig(int bodyUniqueId, Bullet::SoftBodyConfigData& config) {// const {
	ClothInfoCache** clothInfoPtr = m_data->m_clothInfoMap[bodyUniqueId];

	if (clothInfoPtr && *clothInfoPtr) {
		config = (*clothInfoPtr)->m_config;
		return true;
	}
	return false;
}
...
bool PhysicsDirect::getSoftBodyJointInfo(int bodyUniqueId, int jointIndex, Bullet::btSoftBodyJointData& info) {// const {
	ClothInfoCache** clothInfoPtr = m_data->m_clothInfoMap[bodyUniqueId];

	if (clothInfoPtr && *clothInfoPtr && jointIndex >= 0 && jointIndex < (*clothInfoPtr)->m_joints.size()) {
		info = (*clothInfoPtr)->m_joints[jointIndex];
		return true;
	}
	return false;
}

bool PhysicsDirect::getAnchor(int bodyUniqueId, int anchorIndex, Bullet::SoftRigidAnchorData& info) {//const {
	ClothInfoCache** clothInfoPtr = m_data->m_clothInfoMap[bodyUniqueId];

	if (clothInfoPtr && *clothInfoPtr && anchorIndex >= 0 && anchorIndex < (*clothInfoPtr)->m_anchors.size()) {
		info = (*clothInfoPtr)->m_anchors[anchorIndex];
		return true;
	}
	return false;
}

bool PhysicsDirect::getSoftBodyLink(int bodyUniqueId, int linkIndex, Bullet::SoftBodyLinkData& info) {//const {
	ClothInfoCache** clothInfoPtr = m_data->m_clothInfoMap[bodyUniqueId];

	if (clothInfoPtr && *clothInfoPtr && linkIndex >= 0 && linkIndex < (*clothInfoPtr)->m_links.size()) {
		info = (*clothInfoPtr)->m_links[linkIndex];
		return true;
	}
	return false;
}
